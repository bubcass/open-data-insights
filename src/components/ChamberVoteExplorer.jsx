...
function buildDebateUrl(chamber, date, section) {
  const sectionNumber = extractSectionNumber(section);
  if (!date || !sectionNumber) return null;
  return `https://www.oireachtas.ie/en/debates/debate/${chamber.debatePath}/${date}/${sectionNumber}/`;
}
...
function buildVoteFilenameBase(selectedVote, chamber) {
  ...
  return `${chamber.mapDownloadPrefix}_${safeTitle}_${safeDate}`;
}
...
export default function ChamberVoteExplorer({ chamber }) {
  useIframeResize();
  ...
  useEffect(() => {
    async function init() {
      const [seatingRowsRaw, membersResponse, chamberSvgResponse] =
        await Promise.all([
          loadCsv(chamber.seatAssignmentsUrl),
          fetch(chamber.membersUrl, { cache: "no-store" }),
          fetch(chamber.chamberSvgUrl, { cache: "no-store" }),
        ]);

      const membersJson = await membersResponse.json();
      const chamberSvg = await chamberSvgResponse.text();

      const seatingRows = seatingRowsRaw.map((row) => ({
        ...row,
        seat_label: clean(row.seat_label),
        deputy_name: clean(row.deputy_name ?? row.Deputy ?? row.member),
  ...
  const selectedVote = useMemo(() => {
    const vote = votes.find((item) => item.id === selectedVoteId) || null;
    if (!vote) return null;

    return {
      ...vote,
      debateUrl: buildDebateUrl(chamber, vote.date, vote.section),
    };
  }, [chamber, votes, selectedVoteId]);
  ...
  function handleDownloadCurrentVoteCsv() {
    ...
    const filename = `${buildVoteFilenameBase(selectedVote, chamber)}.csv`;
  ...
  function handleDownloadSvg() {
    ...
    downloadSvgFromNode(
      svgNode,
      `${buildVoteFilenameBase(selectedVote, chamber)}.svg`,
    );
    setDownloadsOpen(false);
  }
  ...
  function handleDownloadPng() {
    ...
    downloadPngFromSvgNode(
      svgNode,
      `${buildVoteFilenameBase(selectedVote, chamber)}.png`,
    );
    setDownloadsOpen(false);
  }
              ...
              >
                ...
                <div className="vote-picker__results">
                  {filteredVoteOptions.length === 0 ? (
                    <div className="vote-picker__empty">No matching votes</div>
                  ) : (
        ...
        <section className="panel panel--search">
          ...
          <div className="search-input-wrap">
            <input
              ...
              placeholder={chamber.searchPlaceholder}
              aria-label={chamber.searchAriaLabel}
            />
            {query ? (
              <button
                type="button"
